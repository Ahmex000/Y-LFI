...
func logResult(message string) {
	if resultFile != nil {
		...
		_, err := fmt.Fprintf(resultFile, "%s\n", message)
		if err != nil {
			fmt.Printf("%sError writing to output file: %v%s\n", Red, err, Reset)
		}
		// فرض الكتابة على الملف مباشرةً
		err = resultFile.Sync()
		if err != nil {
			fmt.Printf("%sError syncing output file: %v%s\n", Red, err, Reset)
		}
	}
}
...
func sendNormalRequest(client *http.Client, baseURL string) {
	// نتأكد إن الـ baseURL خالية من أي بارامترات
	parsedURL, err := url.Parse(baseURL)
	if err != nil {
		fmt.Printf("%s[-] Error parsing baseURL %s: %v%s\n", Red, baseURL, err, Reset)
		return
	}
	parsedURL.RawQuery = "" // نزيل أي query parameters
	cleanURL := parsedURL.String()

	req, err := http.NewRequest("GET", cleanURL, nil)
	if err != nil {
		fmt.Printf("%s[-] Error creating normal request for %s: %v%s\n", Red, cleanURL, err, Reset)
		return
	}
	req.Header.Set("User-Agent", userAgents[rand.Intn(len(userAgents))])
