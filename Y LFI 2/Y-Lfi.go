...
func getNextProxy() string {
	...
	for {
		...
		if err == nil && (parsedProxy.Scheme == "http" || parsedProxy.Scheme == "https") {
			return proxy
		}
		if proxyIndex == originalIndex {
			break // لو رجعنا لنفس النقطة، معناها مفيش بروكسي صالح
		}
	}
	return "" // لو مفيش بروكسي صالح، نرجع فارغ
}
...
func performRequestWithRetry(req *http.Request, fullURL string, totalURLs int, totalPayloads int, currentURL int, currentPayload int, headers string, cookies string, reasons []string, timeout int, skipSSLVerify bool) (bool, int) {
	...
	if len(proxies) > 0 {
		proxyURL, err := url.Parse(getNextProxy())
		if err == nil {
			transport.Proxy = http.ProxyURL(proxyURL)
		}
	}
...
func worker(urlChan <-chan string, payloads []string, method string, reqInterval int, requestCount *int, countMutex *sync.Mutex, wg *sync.WaitGroup, headers string, cookies string, timeout int, skipSSLVerify bool, totalURLs int, totalPayloads int, reasons []string) {
	...
	for fullURL := range urlChan {
		...
		if err := limiter.Wait(ctx); err != nil {
			fmt.Printf("%s[-] Rate limiter timeout or error for %s: %v%s\n", Yellow, fullURL, err, Reset)
			cancel()
			continue
		}
		cancel()
		...
		if err != nil {
			fmt.Printf("%s[-] Failed to build request for %s: %v%s\n", Red, fullURL, err, Reset)
			continue
		}

		req = req.WithContext(ctx)

		isVulnerable, responseSize := performRequestWithRetry(req, fullURL, totalURLs, totalPayloads, *requestCount+1, 1, headers, cookies, reasons, timeout, skipSSLVerify)
		if isVulnerable {
			...
			fmt.Printf("%s[+] Request succeeded for %s | Size: %d bytes%s\n", Green, fullURL, responseSize, Reset)
		} else if !hideNotVulnerable {
			fmt.Printf("%s[-] Not vulnerable: %s | Size: %d bytes%s\n", Yellow, fullURL, responseSize, Reset)
		}
		...
		*requestCount++
		...
		if currentRequest%reqInterval == 0 {
			client := createClient(timeout, skipSSLVerify)
			go sendNormalRequest(client, baseEndpoint)
		}
	}
}
